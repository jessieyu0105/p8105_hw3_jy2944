...
One big issue of this dataset is that it contains extensive missing data. In variable precipitation, snowfall, snow depth, maximum temperature, and minimum temperature, the missing data accounts for 5.62%, 14.69%, 22.8%, 43.71%, and 43.71%, respectively. 1372743 observations are missing at least information of one weather variable, which accounts for 52.9% of the total observations (over a half).
